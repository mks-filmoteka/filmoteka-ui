...
export function useFilmSearchParams(resetPageOnChange = false) {
    ...
    const genres = searchParams.get("genres")?.split(",").filter(isGenre) ?? [];
    const countries = searchParams.get("countries")?.split(",").filter(isCountry) ?? [];
    const yearFromUrl = Number(searchParams.get("yearFrom"));
    const yearToUrl = Number(searchParams.get("yearTo"));
    const yearFrom =
        Number.isFinite(yearFromUrl) && yearFromUrl >= MIN_YEAR && yearFromUrl <= MAX_YEAR ? yearFromUrl : undefined;
    ...
    return {
        ...
        setGenres: (values: string[]) => updateParams({ genres: values?.length ? values.join(",") : undefined }),
        ...
        setCountries: (values: string[]) => updateParams({ countries: values?.length ? values.join(",") : undefined }),
        setSort: (sort: { by?: string; dir?: string }[]) => updateParams({ sort: sort.map((s) => `${s.by},${s.dir}`) }),
    };
}
