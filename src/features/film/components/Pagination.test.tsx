...
const getNumberButtons = () => screen.getAllByRole("button").filter((button) => /^\d+$/.test(button.textContent ?? ""));
...
describe("Pagination", () => {
    ...
    it("disables previous navigation on the first page and next navigation on the last page", () => {
        const { rerender } = render(<Pagination page={1} totalPages={3} setPage={vi.fn()} />);
    ...
    it("renders a centered window of up to seven pages", () => {
        ...
        expect(getNumberButtons().map((button) => button.textContent)).toEqual(["7", "8", "9", "10", "11", "12", "13"]);
    });
