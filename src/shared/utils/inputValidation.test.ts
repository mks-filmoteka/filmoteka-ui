...
describe("inputValidation", () => {
    it("keeps valid name and removes unsupported characters", () => {
        expect(sanitizeInput("Test O'Name-Name. #42@", INPUT_RULES.name)).toBe("Test O'Name-Name. ");
    });

    it("keeps valid title and removes unsupported characters", () => {
        expect(sanitizeInput("Test Title: 2 - Test's Cut #[]", INPUT_RULES.title)).toBe("Test Title: 2 - Test's Cut ");
    });
